    ...
    def listServiceGroups(self, additionEndpiont, query):
        ...
        if additionEndpiont == "" and query == {}:
            ...
            with open(self.__PATH__+"/../Data/IoT/"+iota+"/iotagent-setting.json", "r") as f:
                iota_setting = json.load(f)
            header = {'fiware-service': iota, 'fiware-servicepath': iota_setting["iotagent_setting"]["Sub-Service-Group"]}
            try:
            ...
            if subEndpoint=="devices":
                ...
                r=requests.get(iota_setting["iotagent_setting"]["Iot-Agent-Url"]+"/iot/services", headers=header).json()["services"][0]
